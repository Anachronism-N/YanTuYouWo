...
class NoticeListParser:
    ...
    DATE_PATTERNS = [
        ...
        r"\[(\d{4})-(\d{1,2})-(\d{1,2})\]",     # [2026-03-25]
        r"\((\d{4})-(\d{1,2})-(\d{1,2})\)",     # (2026-03-25)
        r"(\d{4})年(\d{1,2})月(\d{1,2})号",     # 2026年3月25号
    ]

    # 综合日期匹配正则（用于启发式解析，覆盖更多格式）
    DATE_REGEX = re.compile(
        r"(\d{4})[-./年](\d{1,2})[-./月](\d{1,2})[日号]?"
    )

    # 分离式日期正则（如月日分散在不同元素中: "2025-06" + "25"）
    DATE_YM_REGEX = re.compile(r"(\d{4})[-./年](\d{1,2})月?$")
    DATE_DAY_REGEX = re.compile(r"^(\d{1,2})[日号]?$")
