...
def infer_program_type(title: str, extracted_type: str | None = None) -> str:
    ...
    本平台聚焦「保研（推免）」。分类遵循「渠道以标题强信号为准」的原则：
    推免 vs 统考（含在职/定向/非全/专项/申请考核制等非推免渠道）这一最关键的
    区分，由标题关键词决定，即使 LLM 返回了「拟录取/招生简章」也会被覆盖——
    因为标题在这点上的信号比 LLM 对正文的判断更可靠。其余细分类型在标题无强
    信号时信任 LLM 的有效分类。

    优先级：
      夏令营 → 推免(预推免/拟录取/入营) → 直博/硕博连读
      → 统考招生（非推免渠道，标题强信号覆盖 LLM）
      → 信任 LLM 有效分类 → 标题兜底推断 → 其他
    ...
    Returns:
        修正后的 program_type
    """
    title_has_summer = bool(re.search(
        r"夏令营|暑期学校|暑期营|暑期学术|秋令营|冬令营|研学营|学术营|学术论坛.*营|暑期.*论坛",
        title,
    ))
    title_has_tuimian = bool(re.search(
        r"推免|推荐免试|预推免|接收.{0,6}推免|免试攻读|优秀大学生.{0,8}(选拔|报名|招募)",
        title,
    ))
    title_has_tongkao = bool(re.search(
    ...
    # 1. 夏令营类（平台核心目标，最高优先级，标题强信号）
    if title_has_summer:
        return "夏令营"

    # 2. 标题含统考/非推免渠道标记 且 无推免标记 → 统考招生（覆盖 LLM）
    #    这条规则解决：LLM 把「在职博士拟录取」「初试考试大纲」误判为
    #    「拟录取/招生简章」的常见错误。
    if title_has_tongkao and not title_has_tuimian:
        return "统考招生"

    # 3. 标题明确推免 → 细分（拟录取/入营名单/预推免）
    if title_has_tuimian:
    ...
    # 5. 标题无强渠道信号时，信任 LLM 的有效分类
    if extracted_type and extracted_type not in ("其他", "null", "None", ""):
        return extracted_type

    # 6. 标题兜底推断（LLM 给了「其他」或空）
    if re.search(r"拟录取|录取.{0,4}名单|待录取", title):
        return "统考招生"  # 无推免上下文的拟录取多为统考/博士
